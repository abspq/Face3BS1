				...
				if 'Bagaimana Anda ingin masuk ke Facebook?' in str(response2) or '/login/?next=' in str(response2):
					...
					if 'https://m.facebook.com/dialog/oauth/?auth_type=rerequest&redirect_uri=' in str(response3.url):
						...
						if 'Sukses!' in str(response6):
							...
							print('')
							print(f"{x}[{h}√{x}]{h} تنشط الكوكيز ارجع شغل الاداة من جديد {x} ");time.sleep(1);exit()
			except Exception as e:
...
def menu(my_name,my_id):
	...
	print(f"""
{X}⥲⥳⥲⥳⥲⥳⥲⥳⥲⥳⥲⥳⥲⥳⥲⥳⥲⥳⥲⥳⥲⥳⥲⥳
{B}⦕ {F}⦲{X} ⦖{F} حسابي التلي {X}⸎ @Y_N_1 
{B}⦕ {F}⦲{X} ⦖{F} قناتي التلي {X}⸎ @NasaTermux
{X}⥲⥳⥲⥳⥲⥳⥲⥳⥲⥳⥲⥳⥲⥳⥲⥳⥲⥳⥲⥳⥲⥳⥲⥳
""")
	print('\x1b[38;5;208mTelegram ᯼ @Y_N_1   ')
	print('')
	print(F+'')
...
def dump_massal():
	...
	try:
		print('')
		print(f'» عدد الفحص {h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
		print('>> Sinyal Loh Kurang Bagus ')
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s <3BƧ> %s/%s » [OK] %s » [CP] %s » %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
